...
def associate_fused_cascade(
    track_boxes: np.ndarray,
    track_means: np.ndarray,
    track_covariances: np.ndarray,
    track_embeddings: np.ndarray,
    track_time_since_update: np.ndarray,
    detection_boxes: np.ndarray,
    detection_embeddings: np.ndarray,
    kf: KalmanFilter,
    max_age: int,
    ...
    if len(track_time_since_update) != num_tracks:
        raise ValueError("track_time_since_update must align with track_boxes")
    ...
    for cascade_level in range(
        1,
        max_age + 2,
    ):
        if not unmatched_detection_indices:
            break

        track_indices = [
            track_idx
            for track_idx in range(num_tracks)
            if (int(track_time_since_update[track_idx]) == cascade_level)
        ]
        ...
        local_matches, _, _ = associate_fused(
            track_boxes=track_boxes[track_indices],
            track_means=track_means[track_indices],
            track_covariances=(track_covariances[track_indices]),
            track_embeddings=(track_embeddings[track_indices]),
            detection_boxes=(detection_boxes[detection_indices]),
            detection_embeddings=(detection_embeddings[detection_indices]),
            kf=kf,
            lambda_motion=(lambda_motion),
        ...
        for (
            ...
            global_track_idx = track_indices[local_track_idx]

            global_detection_idx = detection_indices[local_detection_idx]

            matches.append(
                (
                    global_track_idx,
                    global_detection_idx,
                )
            )

            matched_track_indices.add(global_track_idx)

            matched_detection_indices.add(global_detection_idx)
